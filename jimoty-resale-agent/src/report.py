...
def write_report(items: list[dict], directory: str | Path, report_date: date | None = None) -> Path:
    ...
    lines = [f"# ジモティー仕入れ日報 — {report_date.isoformat()}", "", f"新規/変更分析: {len(items)}件。確定候補 {len(groups['confirmed_candidate'])}件、推論候補 {len(groups['inferred_opportunity'])}件、調査待ち {len(groups['research_pending'])}件、監視 {len(groups['watchlist'])}件、注目見送り {len(groups['reject'])}件。", "", "## TOP candidates", "", "|商品|仕入|純利益|ROI|評価|信頼度|", "|---|---:|---:|---:|---|---|"]
