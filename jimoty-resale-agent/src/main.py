...
def analyse(raw: dict, settings: dict) -> dict | None:
    ...
    return {"listing": item, "title":item.title,"price":item.price,"model":item.model,"year":item.year,"expected":selling,"profit":net,"roi":rate,"score":value,"components":components,"confidence":level,"stars":stars(net,level,liquidity),"status":status,"evidence":evidence,"buyback":next((f"{e.price:,}円" for e in evidence if e.evidence_type=="buyback"), None),"auction":next((f"{e.price:,}円" for e in evidence if e.evidence_type=="sold"), None),"risk":"型番・状態・動作を現物で確認" if inferred else "送料・需要変動", "checks":"銘板の型番、通電/動作、付属品、傷、製造年ラベル" if inferred else "",}
...
def main() -> None:
    ...
    for raw in raw_items:
        ...
        if analysis and storage.is_new_or_changed(analysis["listing"], {k:v for k,v in analysis.items() if k not in {"listing","evidence"}}): storage.save(analysis["listing"], analysis["status"], {k:v for k,v in analysis.items() if k not in {"listing","evidence"}}); results.append(analysis)
